...
##########################Fig_exp_2
{
  ...
  for(j in (1:8)){
    dataresults_Fig_exp2 <- read.table(paste("./DataExp/Exp2/dataresults_exp2_", as.character(stats_exp2[FigList[j], 1]), ".txt", sep = ""))
    
    Fig_j1 <- {plot_ly(dataresults_Fig_exp2, x = ~time, y = ~N1, name = "Species 1",  type = "scatter",  mode = "lines", line = list(color = "#FFD700")) %>%
    ...
    Fig_j2 <- {plot_ly(dataresults_Fig_exp2, x = ~time, y = ~N7, name = "Species 7",  type = "scatter",  mode = "lines", line = list(color = "#8A2BE2")) %>%
