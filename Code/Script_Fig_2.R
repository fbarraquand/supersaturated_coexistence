...
##########################Fig 2 :
{
  ...
  Fig_2a <- {plot_ly(dataresults_2a, x = ~time, y = ~N1, name = "Species 1", type = "scatter", mode = "lines") %>%
      ...
      layout(title = "Time course of the abundances of five species competing for five resources", 
             xaxis = list(title = "Time (days)", 
                        range = c(0, 300)), 
             yaxis = list(title = "Species abundances", 
                        range = c(0, 60)))}
      ...
      layout(title = "The corresponding chaotic attractor. 
           The trajectory is plotted for three of the five species, for the period from t = 1000 to t = 2000 days.", 
             scene = list(xaxis = list(title = "Species 1", range = c(6, 18)), 
                        yaxis = list(title = "Species 3", range = c(15, 40),tickvals = seq(20,40,5)), 
  ...
  Fig_2c<-{plot_ly(dataresults_2c, x = ~time, y = ~Tot, type = "scatter", mode = "lines")%>%
      layout(title = "Time course of total community biomass", 
             xaxis = list(title = "Time (days)", range = c(0, 300)), 
             yaxis = list(title = "Total biomass", range = c(0, 150)))}
  
  
  Fig_2 <- {subplot(Fig_2a, Fig_2b, plotly_empty(), Fig_2c, nrows = 2, margin = 0.11, titleY = T, titleX = T) %>% 
